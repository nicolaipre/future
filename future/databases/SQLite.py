...
class SQLite(Database):
    ...
    def connect(self):
        ...
        if name == ":memory:":
            self.client = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False}, poolclass=StaticPool)
            return self.client
        path = Path(f"{name}.sqlite")
        path.parent.mkdir(parents=True, exist_ok=True)
        self.client = create_engine(f"sqlite:///{path.as_posix()}", connect_args={"check_same_thread": False})
